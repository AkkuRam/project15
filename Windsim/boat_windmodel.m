...
%% Choose a triangle centroid
tri_idx = 1;  % you can select any
Xc = Xb(tri_idx);
Yc = Yb(tri_idx);
Zc = Zb(tri_idx);
...
% Extract surrounding wind vectors (small cube around centroid)
% Find nearest indices in the grid
[~, ix] = min(abs(wind.x - Xc));
[~, iy] = min(abs(wind.y - Yc));
[~, iz] = min(abs(wind.z - Zc));

% Choose a small neighborhood (e.g., +/-1 index)
ix_range = max(1, ix-1):min(numel(wind.x), ix+1);
iy_range = max(1, iy-1):min(numel(wind.y), iy+1);
iz_range = max(1, iz-1):min(numel(wind.z), iz+1);

% Create mesh of points
[Xn, Yn, Zn] = meshgrid(wind.x(ix_range), wind.y(iy_range), wind.z(iz_range));

% Extract wind components from the grid
u_n = wind.u(t0_idx, iy_range, ix_range, iz_range);
v_n = wind.v(t0_idx, iy_range, ix_range, iz_range);
w_n = wind.w(t0_idx, iy_range, ix_range, iz_range);

% Flatten for quiver3
Xn = Xn(:); Yn = Yn(:); Zn = Zn(:);
u_n = u_n(:); v_n = v_n(:); w_n = w_n(:);

% Interpolated wind at centroid
u_interp = u_boat(t0_idx, tri_idx);
v_interp = v_boat(t0_idx, tri_idx);
w_interp = w_boat(t0_idx, tri_idx);

% Plot
figure; hold on;
% Plot surrounding grid vectors
quiver3(Xn, Yn, Zn, u_n, v_n, w_n, 0.5, 'b', 'LineWidth', 1.2);
% Plot interpolated vector at centroid
quiver3(Xc, Yc, Zc, u_interp, v_interp, w_interp, 1, 'r', 'LineWidth', 2);

% Plot centroid
plot3(Xc, Yc, Zc, 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 6);

grid on; axis equal;
xlabel('X'); ylabel('Y'); zlabel('Z');
title(sprintf('Wind vectors around centroid %d at t = %.2f s', tri_idx, wind.t(t0_idx)));
legend('Grid wind', 'Interpolated wind', 'Centroid');
view(45,30);
